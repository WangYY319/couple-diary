...
const Cloud = {
    ...
    if (!this.pairCode) {
      ...
      if (backup && backup.pairCode) {
        ...
        try {
          ...
          const metaR = await fetch(`${this.BASE}/${ns}/meta`, { cache: 'no-cache' });
          if (metaR.ok) {
  ...
  // 读取某角色的最后在线时间（返回 { data, networkError } 区分网络失败和404）
  async getStatus(role) {
    ...
    try {
      ...
      return await r.json();
    } catch (e) { return { _networkError: true }; } // 网络请求失败
  },

  // 检查双方在线状态（3 分钟内活跃视为在线）
  // 返回值：true=在线, false=离线, null=网络错误（不应改变当前显示）
  ...
  async pullDay(dateStr) {
    ...
    try {
      const r = await fetch(this._url(`days/${dateStr}`), { cache: 'no-cache' });
  ...
  async pullAllDays() {
    if (!this.pairCode) return null;
    try {
      const r = await fetch(this._url('days_all'), { cache: 'no-cache' });
      if (!r.ok) return null;
      return await r.json();
    } catch (e) { return null; }
  },
  ...
  async syncAll() {
    ...
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('syncAll', 'syncing');
    ...
    try {
      ...
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('syncAll', 'success');
    } catch (e) {
      console.error('[SYNC] syncAll error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('syncAll', 'error', e.message);
    } finally {
      this.isSyncing = false; // 确保锁总是释放
    }
  },
  ...
  async cleanupOldNamespace() {
    ...
    try {
      const r = await fetch(this._url('photos_list'), { cache: 'no-cache' });
      if (r.ok) {
    ...
    // 2. 清理旧语音数据
    try {
      const r = await fetch(this._url('voices_list'), { cache: 'no-cache' });
      if (r.ok) {
  ...
  async syncToday() {
    ...
    this.isSyncing = true;
    this._syncStartedAt = Date.now();
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('syncToday', 'syncing');
    try {
      const ds = todayStr();
      ...
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('syncToday', 'success');
    } catch (e) {
      console.error('[SYNC] syncToday error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('syncToday', 'error', e.message);
    } finally {
      this.isSyncing = false; // 确保锁总是释放
    }

    // 同步问答和刷词数据（不受isSyncing影响，确保始终执行）
    try {
      await this.syncQuizVocab();
    } catch (e) {
      console.error('[SYNC] syncQuizVocab error:', e);
    }
  },
  ...
  async pushQuizVocab() {
    ...
    try {
      const existingR = await fetch(this._url(`extra/${ds}/${role}`, qaNs), { cache: 'no-cache' });
      if (existingR.ok) {
  ...
  async pullQuizVocab() {
    ...
    const qaNs = `qa-${this._getQuarter()}`;
    try {
      const r = await fetch(this._url(`extra/${ds}/${otherRole}`, qaNs), { cache: 'no-cache' });
      if (!r.ok) {
  ...
  async syncQuizVocab() {
    if (!this.pairCode) return;
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('quizVocab', 'syncing');
    try {
      const ds = todayStr();
    ...
    if (changed) {
      ...
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('quizVocab', 'success');
    } catch (e) {
      console.error('[SYNC] syncQuizVocab error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('quizVocab', 'error', e.message);
    }
  },
  ...
  async syncSubmittedQuestions() {
    ...
    for (const prefix of ['quiz', 'iqa']) {
      try {
        const r = await fetch(this._url('custom/' + prefix + '_submitted'), { cache: 'no-cache' });
        if (!r.ok) continue;
  ...
  async downloadFile(path, nsType) {
    ...
    let meta;
    try {
      const r = await fetch(this._url(`${path}/meta`, nsType), { cache: 'no-cache' });
      if (!r.ok) {
    ...
    for (let i = 0; i < meta.totalChunks; i++) {
      ...
      for (let attempt = 0; attempt <= this.MAX_RETRIES; attempt++) {
        try {
          const r = await fetch(this._url(`${path}/chunk_${i}`, nsType), { cache: 'no-cache' });
          if (r.ok) {
  ...
  async _getPhotoBatches() {
    try {
      const r = await fetch(this._url('photo_batches'), { cache: 'no-cache' });
  ...
  async _isBatchFull(batchName) {
    try {
      const r = await fetch(this._url('photos_list', batchName), { cache: 'no-cache' });
      if (r.ok) {
  ...
  async _getAllRemotePhotoIds() {
    ...
    for (const batch of reg.batches) {
      try {
        const r = await fetch(this._url('photos_list', batch), { cache: 'no-cache' });
        if (r.ok) {
          const list = await r.json();
  ...
  async syncPhotos() {
    if (!this.pairCode) return;
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('photos', 'syncing');
    try {
    ...
    if (uploadFailed && !newPhotos && uploadSuccessCount === 0) {
      ...
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('photos', 'success');
    } catch (e) {
      console.error('[SYNC] syncPhotos error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('photos', 'error', e.message);
    }
  },
  ...
  async syncVoices() {
    if (!this.pairCode) return;
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('voices', 'syncing');
    try {

    // 获取云端录音列表
    let remoteVoices;
    try {
      const r = await fetch(this._url('voices_list', 'voices'), { cache: 'no-cache' });
      if (!r.ok) return;
    ...
    if (newVoices) {
      ...
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('voices', 'success');
    } catch (e) {
      console.error('[SYNC] syncVoices error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('voices', 'error', e.message);
    }
  }
};
...
const CloudSync = {
  ...
  async get(key) {
    ...
    try {
      ...
      console.error(`[CloudSync] get(${key}) network error:`, e.message);
      return null;
    }
  },
  ...
  async syncOnlineDuration() {
    ...
    try {
      ...
      console.error('[SYNC] syncOnlineDuration error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('onlineDuration', 'error', e.message);
    }
  },
  ...
  async syncLetters() {
    if (!Cloud.pairCode) return;
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('letters', 'syncing');
    try {
      ...
      if (!cloudLetters || !Array.isArray(cloudLetters)) {
        ...
        if (typeof SyncDiag !== 'undefined') SyncDiag.record('letters', 'success');
        return;
      }
      ...
      console.error('[SYNC] syncLetters error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('letters', 'error', e.message);
    }
  },
  ...
  async syncPrivateWhispers() {
    ...
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('privateWhispers', 'syncing');
    try {
      ...
      if (!remote || !Array.isArray(remote) || remote.length === 0) {
        ...
        if (typeof SyncDiag !== 'undefined') SyncDiag.record('privateWhispers', 'success');
        return;
      }

      // 双向合并
      const local = PrivateWhisper._getList();
      const localMap = new Map(local.map(x => [`${x.ts}_${x.by}`, x]));
      ...
      // 1. 合并云端数据到本地（拉取对方投递的絮语 + 更新已有条目属性）
      for (const item of remote) {
        if (item && item.text) {
          ...
          if (!localItem) {
            ...
            // 已有条目：检查属性是否有更新（高亮状态等）
            let itemChanged = false;
            if (!!item.highlighted !== !!localItem.highlighted) {
              localItem.highlighted = item.highlighted;
              itemChanged = true;
            }
      ...
      if (newItems.length > 0 || deletedCount > 0 || updatedCount > 0) {
        ...
        console.log('[SYNC] private_whispers: pulled', newItems.length, 'new items, updated', updatedCount, 'items, removed', deletedCount, 'deleted items');
      }

      // 3. 推送合并后的完整数据回云端
      const finalList = (newItems.length > 0 || deletedCount > 0 || updatedCount > 0)
      ...
      if (hasLocalOnly || newItems.length > 0 || deletedCount > 0 || updatedCount > 0) {
      ...
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('privateWhispers', 'error', e.message);
    }
  },
  ...
  async syncMindList() {
    ...
    if (typeof SyncDiag !== 'undefined') SyncDiag.record('mindList', 'syncing');
    try {
      ...
      if (!remote || !Array.isArray(remote) || remote.length === 0) {
        ...
        if (typeof SyncDiag !== 'undefined') SyncDiag.record('mindList', 'success');
        return;
      }
      ...
      console.error('[SYNC] syncMindList error:', e);
      if (typeof SyncDiag !== 'undefined') SyncDiag.record('mindList', 'error', e.message);
    }
  }
};
...
const SyncDiag = {
  // 各模块同步状态记录
  status: {
    syncToday: { lastTime: 0, status: 'idle', error: null },
    syncAll: { lastTime: 0, status: 'idle', error: null },
    onlineDuration: { lastTime: 0, status: 'idle', error: null },
    privateWhispers: { lastTime: 0, status: 'idle', error: null },
    letters: { lastTime: 0, status: 'idle', error: null },
    quizVocab: { lastTime: 0, status: 'idle', error: null },
    mindList: { lastTime: 0, status: 'idle', error: null },
    photos: { lastTime: 0, status: 'idle', error: null },
    voices: { lastTime: 0, status: 'idle', error: null },
  },
  ...
  // 记录模块同步状态
  record(module, status, error) {
    if (!this.status[module]) this.status[module] = { lastTime: 0, status: 'idle', error: null };
    this.status[module].lastTime = Date.now();
    this.status[module].status = status;
    this.status[module].error = error || null;

    // 记录日志
    const timeStr = new Date().toLocaleTimeString('zh-CN', { hour12: false });
    const level = status === 'error' ? 'error' : status === 'success' ? 'success' : 'info';
    const msg = `[${timeStr}] ${module}: ${status}${error ? ' - ' + error : ''}`;
  ...
  addLog(msg, level = 'info') {
    this.logs.push({ msg, level, time: Date.now() });
  ...
  // 打开面板
  open() {
    const mask = document.getElementById('syncDiagMask');
    if (mask) mask.classList.add('show');
    this.refreshUI();
    this.addLog('诊断面板已打开', 'info');
  },

  // 关闭面板
  close(e) {
    if (e && e.target && e.target.id !== 'syncDiagMask') return;
    const mask = document.getElementById('syncDiagMask');
  ...
  refreshUI() {
    ...
    // 更新锁状态
    const lockStatusEl = document.getElementById('syncDiagLockStatus');
    const lockInfoEl = document.getElementById('syncDiagLockInfo');
    if (lockStatusEl && typeof Cloud !== 'undefined') {
      if (Cloud.isSyncing) {
        const lockedFor = Cloud._syncStartedAt ? Math.floor((Date.now() - Cloud._syncStartedAt) / 1000) : 0;
        lockStatusEl.textContent = '🔴 已锁定';
        lockStatusEl.style.color = '#e53935';
        if (lockInfoEl) {
          lockInfoEl.innerHTML = `已锁定 <b>${lockedFor}</b> 秒<br>锁定开始：${Cloud._syncStartedAt ? new Date(Cloud._syncStartedAt).toLocaleTimeString('zh-CN', { hour12: false }) : '未知'}`;
        }
      } else {
        lockStatusEl.textContent = '🟢 未锁定';
        lockStatusEl.style.color = '#43a047';
        if (lockInfoEl) {
          const lastSync = Cloud.lastSyncAt ? new Date(Cloud.lastSyncAt).toLocaleTimeString('zh-CN', { hour12: false }) : '从未同步';
          lockInfoEl.innerHTML = `上次同步：${lastSync}`;
        }
      }
    }
    ...
    // 更新各模块状态
    const moduleMap = {
      'diag-syncToday': 'syncToday',
      'diag-onlineDuration': 'onlineDuration',
      'diag-privateWhispers': 'privateWhispers',
      'diag-letters': 'letters',
      'diag-quizVocab': 'quizVocab',
      'diag-mindList': 'mindList',
      'diag-photos': 'photos',
      'diag-voices': 'voices',
    };

    for (const [elId, moduleKey] of Object.entries(moduleMap)) {
      const el = document.getElementById(elId);
      if (!el) continue;
      const s = this.status[moduleKey];
      if (!s || s.status === 'idle') {
        el.textContent = '未同步';
        el.className = 'sync-diag-status idle';
      } else {
        el.className = `sync-diag-status ${s.status}`;
        const timeAgo = s.lastTime ? Math.floor((Date.now() - s.lastTime) / 1000) : 0;
    ...
    // 更新日志
    const logEl = document.getElementById('syncDiagLog');
    if (logEl) {
  ...
  // 强制全量同步
  async forceSyncAll() {
    this.addLog('手动触发全量同步...', 'info');
    if (typeof Cloud !== 'undefined') {
      Cloud.isSyncing = false; // 先解锁
      try {
        await Cloud.syncAll();
        this.addLog('全量同步完成', 'success');
      } catch (e) {
        this.addLog('全量同步失败: ' + e.message, 'error');
      }
    }
    // 同时触发其他模块同步
    if (typeof CloudSync !== 'undefined') {
  ...
  async forceSyncToday() {
    this.addLog('手动同步今日数据...', 'info');
    if (typeof Cloud !== 'undefined') {
      Cloud.isSyncing = false;
      try {
        await Cloud.syncToday();
        this.addLog('今日数据同步完成', 'success');
      } catch (e) {
        this.addLog('今日数据同步失败: ' + e.message, 'error');
      }
    }
    this.refreshUI();
  },
  ...
  // 强制同步私密絮语
  async forceSyncWhispers() {
    this.addLog('手动同步私密絮语...', 'info');
    if (typeof CloudSync !== 'undefined') {
      try {
        await CloudSync.syncPrivateWhispers();
        this.addLog('私密絮语同步完成', 'success');
      } catch (e) {
        this.addLog('私密絮语同步失败: ' + e.message, 'error');
      }
    }
    this.refreshUI();
  },
...
const App = {
  ...
  enterApp() {
    ...
    this.updateNav();
    // 显示当前版本号
    const verEl = document.getElementById('navVersion');
    if (verEl && typeof Setting !== 'undefined' && Setting.VERSION_LOG) {
      verEl.textContent = Setting.VERSION_LOG[0].v;
    }
    Calendar.render();
    Cards.renderAll();
    Cards.updateRolePermissions();
  ...
  async forceSync() {
    ...
    try {
      ...
      const todayData = Store.getDay(todayStr());
      console.log('[FORCE-SYNC] Today data:', JSON.stringify(todayData));

      // 汇总同步结果
      const pwList = typeof PrivateWhisper !== 'undefined' ? PrivateWhisper._getList() : [];
      const mlCount = typeof MindList !== 'undefined' && MindList.items ? MindList.items.length : 0;
      const letterCount = typeof LetterBox !== 'undefined' && LetterBox.letters ? LetterBox.letters.length : 0;
      console.log('[FORCE-SYNC] Summary:', {
        greet: todayData?.greet ? `TAO:${!!todayData.greet.tao} YAN:${!!todayData.greet.yan}` : 'none',
        words: todayData?.words ? `TAO:${todayData.words.tao?.length||0}字 YAN:${todayData.words.yan?.length||0}字` : 'none',
        wish: todayData?.wish ? `TAO:${todayData.wish.tao?.length||0}字 YAN:${todayData.wish.yan?.length||0}字` : 'none',
        night: todayData?.night ? `TAO:${!!todayData.night.tao} YAN:${!!todayData.night.yan}` : 'none',
        privateWhispers: pwList.length + '条',
        mindList: mlCount + '条',
        letters: letterCount + '封',
      });
...
const Pair = {
  ...
  // 快捷登录：直接用历史配对码加入
  // 如果云端 meta 不存在（可能被清理），用原配对码重建 meta，保留原有数据命名空间
  async quickJoin(code, role) {
    ...
    if (r._notFound) {
      ...
      if (createResult.ok) {
        Cloud.isFreshLogin = true; // 标记为新登录，触发完整同步
        App.enterAfterPair(role);
        return;
      }
...
const Setting = {
  ...
  VERSION_LOG: [
    ...
    { v: 'v134', date: '2026-09-14', changes: '修复私密絮语同步:已有条目属性(高亮状态)双向合并更新/CloudSync.get增加错误日志(区分404与网络异常)/forceSync同步结果控制台详细汇总输出' },
    ...
    { v: 'v115', date: '2026-08-24', changes: '性能优化:移除vocab-data.js同步阻塞/非关键模块requestIdleCallback延迟初始化/消除双重初始化/云同步错峰执行/visibilitychange防抖' },
  ...
  async refreshStatus() {
    ...
    // 防竞态：如果上一次 refreshStatus 还在执行中，跳过本次
    if (this._refreshingStatus) return;
    this._refreshingStatus = true;
    // 自己必然在线（始终设为在线，不被云端数据覆盖）
    this.setStatusDot(App.currentRole, true);
    try {
      const st = await Cloud.checkOnlineStatus();
      // 只更新对方的状态，自己的状态保持在线
      const otherRole = App.currentRole === 'TAO' ? 'YAN' : 'TAO';
      const otherStatus = otherRole === 'TAO' ? st.tao : st.yan;
      // null = 网络错误，不改变当前显示；true/false = 确实在线/离线
      if (otherStatus !== null) {
        this.setStatusDot(otherRole, otherStatus);
      }
      // 检测双方同时在线（自己始终在线，只需判断对方）
      const bothOnline = otherStatus === true;
      ...
      <div class="quiz-partner-card" onclick="event.stopPropagation()">
        ...
        <div class="quiz-partner-body">
          <div style="margin-bottom:12px;">
            ...
            <input id="sweetSubmitText" type="text" maxlength="30" placeholder="例：遇见你是我最幸运的事"
            ...
            <div style="margin-top:6px;font-size:11px;color:#bbb;line-height:1.6;">
              💡 参考：想和你一起看日落 / 你是我的小确幸 / 和你在一起的每一天都很甜
            </div>
          </div>
      ...
      <div class="quiz-partner-card" onclick="event.stopPropagation()">
        ...
        <div class="quiz-partner-body">
          <div style="margin-bottom:12px;">
            ...
            <input id="privateWhisperSubmitText" type="text" maxlength="30" placeholder="例：今晚月色真美，想你了"
              style="width:100%;padding:8px 10px;border:1px solid #ddd;border-radius:8px;font-size:14px;box-sizing:border-box;" autocomplete="off" />
            <div style="margin-top:6px;font-size:11px;color:#bbb;line-height:1.6;">
              💡 参考：突然好想抱抱你 / 你身上好香想赖着 / 下次见面要亲你一下
            </div>
          </div>
...
const RandomQA = {
  ...
  QUESTION_BANK: [
    ...
    { q: '你更想学的舞蹈类型？', a: ['街舞爵士酷炫帅气', '拉丁华尔兹优雅'] },
    { q: '你更喜欢的乐器音色？', a: ['吉他民谣温暖', '钢琴古典优雅'] },
    { q: '你更想尝试的户外运动？', a: ['冲浪桨板水上漂', '攀岩抱陆上爬'] },
    { q: '你更喜欢的创作方式？', a: ['画画涂鸦色彩表达', '写诗写歌文字创作'] },
    ...
    { q: '你更想学的摄影风格？', a: ['人像写真捕捉情绪', '风景大片壮阔山河'] },
    { q: '你更喜欢的桌游类型？', a: ['策略推理烧脑型', '欢乐聚会互动型'] },
    { q: '你更想尝试的手工？', a: ['陶艺捏泥土治愈感', '木工做小物成就感'] },
    { q: '你更喜欢的唱歌场合？', a: ['KTV嗨唱一展歌喉', '家里哼着自娱自乐'] },
    { q: '你更想去的音乐节？', a: ['摇滚电音燥起来', '民谣爵士治愈系'] },
    { q: '你更喜欢的绘画风格？', a: ['水彩插画小清新', '油画丙烯厚重感'] },
    { q: '你更想学的烘焙？', a: ['蛋糕甜点精致型', '面包欧包实在型'] },
    { q: '你更喜欢的运动竞技？', a: ['羽毛球乒乓对决', '篮球足球团队赛'] },
    { q: '你更想尝试的冒险？', a: ['蹦极跳伞肾上腺素', '深潜探秘海底世界'] },
    { q: '你更喜欢的短视频类型？', a: ['搞笑段子沙雕日常', '美食探店种草推荐'] },
    { q: '你更想学的编程语言？', a: ['Python数据分析', '前端开发做网页'] },
    { q: '你更喜欢的棋类？', a: ['围棋象棋深思熟虑', '跳棋五子棋轻松玩'] },
    { q: '你更想尝试的茶艺？', a: ['功夫茶仪式感满满', '袋泡茶方便快捷'] },
    { q: '你更喜欢的运动强度？', a: ['有氧慢跑出汗舒服', '力量训练增肌塑形'] },
    { q: '你更想学的剪辑技能？', a: ['短视频快节奏搞笑', 'Vlog记录生活日常'] },
    { q: '你更喜欢的收藏爱好？', a: ['盲盒手办潮玩系列', '邮票硬币复古收藏'] },
    { q: '你更想尝试的 DIY？', a: ['手工皮具做钱包', '滴胶树脂做饰品'] },
    { q: '你更喜欢的音乐风格？', a: ['说唱Hip-Hop节奏感', '轻音乐纯音乐放松'] },
    { q: '你更想去的体验馆？', a: ['密室逃脱烧脑刺激', '手作馆DIY治愈放松'] },
    { q: '你更喜欢的露营方式？', a: ['帐篷野营亲近自然', '房车露营舒适型'] },
    { q: '你更想学的花艺？', a: ['插花艺术东方禅意', '花束包扎西式浪漫'] },
  ],
...
const IntimateQA = createQAModule({
  ...
  bank: [
    ...
    // 新增：撒娇心动类
    { q: '对方撒娇时你更想？', a: ['忍不住宠着对方', '故意逗逗再满足'] },
    { q: '你更心动的瞬间？', a: ['对方认真看着你笑', '对方不经意靠近你'] },
    { q: '你更喜欢的暧昧方式？', a: ['若即若离撩人心弦', '直球出击热烈表达'] },
    { q: '对方靠很近时你更？', a: ['心跳加速假装淡定', '顺势靠得更近一点'] },
    { q: '你更想被怎么撩？', a: ['耳边的低声情话', '突然的壁咚靠近'] },
    { q: '你更喜欢的撒娇方式？', a: ['软软地叫对方昵称', '拉拉衣角蹭蹭肩膀'] },
    { q: '你更心动的眼神？', a: ['深情注视不移开', '偷偷看你被发现'] },
    { q: '你更喜欢的亲密距离？', a: ['十指紧扣贴着走', '环腰搂肩占有感'] },
    { q: '对方偷偷亲你一下？', a: ['脸红心跳装没事', '立刻亲回去报复'] },
    { q: '你更想和对方？', a: ['缩在沙发上看电影', '并排躺着刷手机'] },
    // 新增：暧昧互动类
    { q: '你更喜欢对方哪种撩？', a: ['不经意的肢体接触', '说让人脸红的话'] },
    { q: '你更心动的称呼？', a: ['被叫专属的昵称', '被叫宝贝亲爱的'] },
    { q: '你更喜欢的约会氛围？', a: ['昏暗灯光暧昧气息', '阳光下甜蜜逛街'] },
    { q: '对方突然抱住你？', a: ['反手抱紧不撒手', '挣扎一下再抱住'] },
    { q: '你更想尝试的亲密？', a: ['面对面额头相抵', '从背后被环抱'] },
    { q: '你更喜欢被怎么哄睡？', a: ['轻拍背哼着歌', '讲故事直到睡着'] },
    { q: '你更心动的声音？', a: ['对方刚睡醒的嗓音', '对方叫你昵称的声音'] },
    { q: '你更想要的早安？', a: ['被亲额头叫醒', '被捏脸说懒猪'] },
    { q: '你更喜欢的晚安？', a: ['被抱紧说晚安', '额头一个吻入睡'] },
    { q: '对方盯着你看你更？', a: ['害羞低头躲开', '回盯比谁先笑'] },
    // 新增：暧昧场景类
    { q: '雨天只有一把伞你更？', a: ['共撑一把靠很近', '把伞给对方自己淋'] },
    { q: '冬天对方手冷你更？', a: ['直接牵手放口袋', '搓热自己的手捂上'] },
    { q: '你更心动的意外？', a: ['对方突然出现你面前', '收到没预告的礼物'] },
    { q: '你更喜欢的拥抱？', a: ['紧紧勒住喘不过气', '温柔环住轻轻拍背'] },
    { q: '你更想要的牵手方式？', a: ['十指紧扣掌心贴', '对方大手包住你的'] },
    { q: '看电影时对方拉你手？', a: ['顺势靠在肩上', '偷偷十指紧扣'] },
    { q: '你更想被怎么叫醒？', a: ['被亲脸颊弄醒', '被捏鼻子弄醒'] },
    { q: '你更喜欢的亲密称呼？', a: ['宝宝乖乖甜到腻', '专属外号只有你知道'] },
    { q: '你更心动对方做什么？', a: ['不经意帮你理头发', '弯腰帮你系鞋带'] },
    { q: '对方喝醉了对你撒娇？', a: ['宠溺地由着对方', '趁机逗到脸红'] },
    // 新增：仪式感与浪漫类
...
const PoliCard = {
  // 考研政治重要定义
  POLI: [
    '马克思主义是关于无产阶级和人类解放的科学理论体系。',
    '物质是标志客观实在的哲学范畴，能为意识所复写反映。',
    '意识是物质世界长期发展的产物，是人脑的机能和属性。',
    '实践是人类能动地改造世界的物质活动，是认识的基础。',
    '矛盾同一性指矛盾双方相互依存、相互转化的性质。',
    '矛盾的普遍性即矛盾无处不在无时不有，是事物发展的动力。',
    '量变是事物数量的增减和场所的变更，是渐进的连续的变化。',
    '质变是事物根本性质的变化，是渐进过程的中断即飞跃。',
    '否定之否定规律揭示事物发展的方向和道路是前进上升的。',
    '认识是在实践基础上主体对客体的能动的反映。',
    '真理是标志主观同客观相符合的哲学范畴，具有客观性。',
    '实践是检验真理的唯一标准，因其具有直接现实性。',
    '社会存在是社会物质生活条件，包括地理人口和生产方式。',
    '社会意识是社会精神生活的总和，是社会存在的反映。',
    '生产关系是人们在物质生产过程中结成的社会关系。',
    '生产力是人类改造自然获取物质生活资料的能力。',
    '经济基础是占统治地位的生产关系的总和。',
    '上层建筑是建立在经济基础之上的意识形态及相应制度。',
    '人民群众是历史的创造者，是推动社会发展的决定力量。',
    '商品是为交换而生产的劳动产品，具有使用价值和价值。',
    '抽象劳动是撇开具体形式的无差别的一般人类劳动。',
    '价值是凝结在商品中的无差别的人类劳动。',
    '剩余价值是雇佣工人创造的被资本家无偿占有的价值。',
    '资本是能够带来剩余价值的价值。',
    '不变资本是转化为生产资料的那部分资本，不增殖价值。',
    '可变资本是转化为劳动力的那部分资本，能增殖价值。',
    '资本积累是剩余价值转化为资本的过程。',
    '资本主义基本矛盾是生产社会化与生产资料私人占有之间的矛盾。',
    '新民主主义革命总路线是无产阶级领导的反帝反封建革命。',
    '三大作风是理论联系实际、密切联系群众、批评与自我批评。',
    '实事求是是毛泽东思想的精髓和根本出发点。',
    '群众路线是一切为了群众一切依靠群众从群众中来到群众中去。',
    '独立自主是中国革命和建设的根本立足点。',
    '社会主义本质是解放生产力发展生产力消灭剥削消除两极分化。',
    '社会主义初级阶段是生产力不发达基础上的不发达阶段。',
    '四项基本原则是立国之本，改革开放是强国之路。',
    '以经济建设为中心是兴国之要，四项基本原则是立国之本。',
    '三个代表重要思想核心是代表先进生产力先进文化和人民利益。',
    '科学发展观第一要义是发展，核心是以人为本。',
    '中国梦的本质是国家富强民族振兴人民幸福。',
    '全面深化改革总目标是完善和发展中国特色社会主义制度。',
    '全面依法治国总目标是建设中国特色社会主义法治体系。',
    '新时代我国社会主要矛盾是发展不平衡不充分与美好生活的矛盾。',
    '供给侧结构性改革主攻方向是减少无效供给扩大有效供给。',
    '社会主义核心价值观包含国家社会个人三个层面的价值要求。',
    '高质量发展是创新成为第一动力协调成为内生特点的发展。',
    '新发展理念是创新协调绿色开放共享的发展理念。',
    '一带一路是共建丝绸之路经济带和21世纪海上丝绸之路。',
    '人类命运共同体是建设持久和平普遍安全共同繁荣的世界。',
    '党的领导是中国特色社会主义最本质的特征。',
    '全过程人民民主是全链条全方位全覆盖的民主。',
    '中国式现代化是人口规模巨大的现代化全体人民共同富裕的现代化。',
    '两个确立是确立党中央核心和指导思想在全党的指导地位。',
    '四个意识是政治意识大局意识核心意识看齐意识。',
    '四个自信是道路自信理论自信制度自信文化自信。',
    '两个维护是坚决维护党中央的核心和党中央权威集中统一领导。',
    '全过程人民民主实现了过程民主和成果民主形式和实质民主统一。',
    '党的百年奋斗历史经验包括十个坚持，其中首要为坚持党的领导。',
    '坚持人民至上是党百年奋斗的根本立场和价值取向。',
    '坚持自我革命是党跳出历史周期率的第二个答案。',
    '坚持胸怀天下是党在全球视野下的使命担当。',
    '国家安全是民族复兴的根基，社会稳定是国家强盛的前提。',
    '总体国家安全观以人民安全为宗旨以政治安全为根本。',
    '全面从严治党是党永葆生机活力的根本保证。',
    '不敢腐不能腐不想腐一体推进是反腐败斗争的基本方针。',
    '坚持和完善社会主义基本经济制度，坚持两个毫不动摇。',
    '按劳分配为主体多种分配方式并存是社会主义分配制度。',
    '社会主义市场经济体制是市场在资源配置中起决定性作用。',
    '乡村振兴战略总要求是产业兴旺生态宜居乡风文明治理有效生活富裕。',
    '区域协调发展战略包括京津冀协同发展长江经济带粤港澳大湾区等。',
    '创新驱动发展战略是建设科技强国质量强国航天强国网络强国。',
    '人才强国战略是聚天下英才而用之，加快建设人才强国。',
    '坚持绿色发展是发展观的一场深刻革命，坚持人与自然和谐共生。',
    '碳达峰碳中和是中国对国际社会的庄严承诺，推动绿色低碳发展。',
    '坚持对外开放的基本国策，推动形成全面开放新格局。',
    '坚持一国两制方针，推进祖国统一大业。',
    '构建新发展格局以国内大循环为主体国内国际双循环相互促进。',
    '坚持党对一切工作的领导是最高政治原则。',
    '人民代表大会制度是我国的根本政治制度。',
    '中国共产党领导的多党合作和政治协商制度是我国基本政治制度。',
    '民族区域自治制度是我国基本政治制度，维护民族团结。',
    '基层群众自治制度是我国基本政治制度，保障人民当家作主。',
  ],
  ...
  init() {
    const dateStr = todayStr();
    const cached = Store.get(`poli_${dateStr}`, null);
    if (cached !== null && cached >= 0 && cached < this.POLI.length) {
      this._currentIndex = cached;
    } else {
      const indices = this._seededShuffle(
        this.POLI.map((_, i) => i),
      ...
      const yesterdayStr = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
      const yesterdayCache = Store.get(`poli_${yesterdayStr}`, null);
      let yesterdayIndex = -1;
      if (yesterdayCache !== null && yesterdayCache >= 0) {
        yesterdayIndex = yesterdayCache;
      }
      this._currentIndex = indices.find(i => i !== yesterdayIndex) ?? indices[0];
      Store.set(`poli_${dateStr}`, this._currentIndex);
    }
    this._render();
  },
  ...
  refresh() {
    if (this.POLI.length <= 1) {
      ...
      let idx;
      do {
        idx = Math.floor(Math.random() * this.POLI.length);
      } while (idx === this._currentIndex);
      this._currentIndex = idx;
    }
    const dateStr = todayStr();
  ...
  _render() {
    const el = document.getElementById('poliContent');
    if (!el) return;
    el.textContent = this.POLI[this._currentIndex] || this.POLI[0];
  }
};
